...
export default function (inputFile) {
  ...
  const { visible, scenicScore } = trees.map.reduce(
    ({ visible, scenicScore }, tree) => {
      ...
      if (
        [0, trees.size.x - 1].includes(x) ||
        [0, trees.size.y - 1].includes(y)
      ) {
      ...
      const rowBefore = Array.from(
        { length: x },
        (_, i) => trees.map[trees.getPositionIndex(i, y)].value
      ).reverse();
      const rowAfter = Array.from(
        { length: trees.size.x - x - 1 },
        (_, i) => trees.map[trees.getPositionIndex(i + x + 1, y)].value
      );
      const colBefore = Array.from(
        { length: y },
        (_, i) => trees.map[trees.getPositionIndex(x, i)].value
      ).reverse();
      const colAfter = Array.from(
        { length: trees.size.y - y - 1 },
        (_, i) => trees.map[trees.getPositionIndex(x, i + y + 1)].value
      );
      ...
      return {
        ...
        scenicScore:
          treeScenicScore > scenicScore ? treeScenicScore : scenicScore,
      };
    },
