import { getStacksTopRow, processInstructions } from "../../utils/2022/crates";
...
export default function (inputFile) {
  ...
  const oneByOne = processInstructions(
    instructions,
    stacks,
    (moved, itemsToMove, from, to) => {
  ...
  const allAtOnce = processInstructions(
    instructions,
    stacks,
    (moved, itemsToMove, from, to) => {
      // Move all items from the source crate to the target at once
      const toMove = moved[from - 1].splice(
        moved[from - 1].length - itemsToMove
      );
